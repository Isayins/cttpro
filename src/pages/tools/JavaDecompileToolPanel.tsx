...
export function JavaDecompileToolPanel({
  ...
  return (
    <div className="space-y-6">
      {error ? <Alert type="error" showIcon icon={<WarningOutlined />} message="Java 反编译失败" description={error} /> : null}
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={10}>
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                <input id="class-file-input" type="file" accept=".class" onChange={onFileUpload} className="hidden" />
              </div>
            </div>
            ...
            {decompiling ? (
              <div className="flex h-full min-h-[388px] items-center justify-center text-gray-400">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-xs">当前会优先显示后端返回的反编译输出</p>
                </div>
              </div>
            ) : output ? (
