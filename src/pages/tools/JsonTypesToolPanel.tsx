    ...
    <div className="space-y-6">
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          ...
          <Input value={rootName} onChange={(event) => onRootNameChange(event.target.value)} placeholder="例如 ApiResponse / UserProfile" />
          <label className="mb-2 mt-4 block text-sm font-medium text-gray-700">JSON 示例</label>
          <TextArea rows={10} value={input} onChange={(event) => onInputChange(event.target.value)} placeholder='{"id":1,"name":"Alice","tags":["admin"]}' />
