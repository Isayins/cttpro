...
实用工具页在浏览器本地完成大部分转换。正则测试通过限时 Worker 执行，二维码解析仅在使用时加载并限制图片大小与分辨率；时间戳支持自动、秒和毫秒三种单位；随机转盘使用浏览器安全随机数无偏抽取结果，再播放对应落点动画，公开地址 `/wheel` 无需登录即可使用；它支持设置统计次数、记录本轮结果、撤销上次结果、保存历史轮次、复制摘要和导出 CSV，并在刷新后恢复；重复选项会被拒绝。公开地址 `/rps` 提供本地双人猜拳桌：两位玩家加入后随机获得身份牌，双方锁定石头、剪刀或布后倒计时 3 秒揭晓并累计比分。为避免凭据或业务数据残留，只有颜色、Cron、UUID 和转盘记录会跨会话保存在浏览器中，其他工具历史仅在当前页面会话内可用。
...
- 后台「二维码管理」支持链接跳转和自定义 HTML/JavaScript 页面；HTML 页面在不具备同源和顶层跳转权限的沙箱中运行，并可通过内置模板展示长期累计扫码次数。扫码请求携带访问标识，网络重试不会重复累计；扫码明细仍按 90 天保留，累计次数不随明细清理而减少。列表仅加载摘要，编辑时再按需读取 HTML 源码。
- 邮箱取码支持按邮箱分页浏览历史邮件并查看纯文本正文；正文会保留换行，脚本和样式内容不会展示，超长正文会明确标记为已截断。
...
- 登录页支持通过注册邮箱验证码重置密码；通过重置流程或个人资料页修改密码成功后，都会使该账号当前登录会话失效并要求重新登录。
- 个人资料的账户安全区支持使用当前密码和新邮箱验证码更换登录邮箱；成功后旧会话立即失效，需要重新登录。
- 更换邮箱验证码在有效期内最多允许连续输错 5 次，达到上限后必须重新获取验证码。
- 个人资料昵称最多 50 个字符、简介最多 500 个字符；个人头像通过站内图片上传设置，单张不超过 5MB，可删除并回退为昵称首字头像。
- 个人资料页展示真实的发帖与收藏总数，并各自只加载最近 3 条作为预览；更新公开资料不会重复刷新内容和登录记录。
- 支付首次完成及发货结果会写入站内通知；已完成的邮箱发货订单可在「我的订单」自助补发，补发间隔为 60 秒；CDK 缺货订单补充库存后会在补发时自动分配。
- 消息中心默认加载最近 20 条个人消息，并支持继续加载历史消息；未读数量会在页面可见时自动刷新，论坛和订单通知可直接打开对应内容，站点公告保持独立展示。
