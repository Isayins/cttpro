...
实用工具页在浏览器本地完成大部分转换。正则测试通过限时 Worker 执行，二维码解析仅在使用时加载并限制图片大小与分辨率；时间戳支持自动、秒和毫秒三种单位；随机转盘使用浏览器安全随机数无偏抽取结果，再播放对应落点动画，公开地址 `/wheel` 无需登录即可使用；它支持设置统计次数、记录本轮结果、撤销上次结果、保存历史轮次、复制摘要和导出 CSV，并在刷新后恢复；重复选项会被拒绝。公开地址 `/rps` 提供房间列表和房间码跨设备联机猜拳桌：创建者生成 6 位房间码，另一位玩家可以从大厅选择空桌或在自己的设备输入房间码加入；公开房可直接加入，好友房需要房主审批，加密房还需要密码并等待房主审批。两位玩家会分别随机获得身份牌，双方锁定石头、剪刀或布后由服务端倒计时 3 秒、统一揭晓并累计比分。联机同步当前通过 REST 轮询完成，不依赖 WebSocket；出拳在揭晓前只返回“已锁定”状态，不会把具体手势发给对方设备。为避免凭据或业务数据残留，只有颜色、Cron、UUID 和转盘记录会跨会话保存在浏览器中，其他工具历史仅在当前页面会话内可用。
