...
实用工具页在浏览器本地完成大部分转换。正则测试通过限时 Worker 执行，二维码解析仅在使用时加载并限制图片大小与分辨率；时间戳支持自动、秒和毫秒三种单位；随机转盘使用浏览器安全随机数无偏抽取结果，再播放对应落点动画，支持设置统计次数、记录本轮结果、复制摘要和导出 CSV，并在刷新后恢复当前轮次；重复选项会被拒绝。为避免凭据或业务数据残留，只有颜色、Cron、UUID 和转盘记录会跨会话保存在浏览器中，其他工具历史仅在当前页面会话内可用。
